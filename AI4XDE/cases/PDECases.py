...
class Diffusion(PDECases):
    def __init__(self, 
                 ...
                 activation='tanh', 
                 initializer='Glorot normal'):
        super().__init__(name='Diffusion', NumDomain=NumDomain, use_output_transform=use_output_transform, layer_size=layer_size, activation=activation, initializer=initializer)
    ...
    def gen_data(self):
        if self.use_output_transform:
            data = dde.data.TimePDE(self.geomtime, self.pde, [], num_domain=self.NumDomain, solution=self.sol, num_test=10000)
        else:
            ...
            icbc = [bc, ic]
            data = dde.data.TimePDE(self.geomtime, self.pde, icbc, num_domain=self.NumDomain, num_boundary=20, num_initial=10, solution=self.sol, num_test=10000)
        return data
...
class Fractional_Poisson_2D(PDECases):
    def __init__(self, 
                 ...
                 initializer='Glorot normal'):
        self.alpha = 1.8
        super().__init__(name='Fractional Poisson equation in 2D', NumDomain=NumDomain, use_output_transform=True, layer_size=layer_size, activation=activation, initializer=initializer)
    ...
    def gen_pde(self):
        ...
        def fpde(x, y, int_mat):
            ...
            if isinstance(int_mat, (list, tuple)) and len(int_mat) == 3:
                ...
                lhs = bkd.matmul(bkd.from_numpy(int_mat), bkd.from_numpy(y))
            lhs = lhs[:, 0]
            lhs *= gamma((1 - self.alpha) / 2) * gamma((2 + self.alpha) / 2) / (2 * np.pi ** 1.5)
            x = x[: bkd.size(lhs)]
            rhs = (
                2 ** self.alpha
                * gamma(2 + self.alpha / 2)
                * gamma(1 + self.alpha / 2)
                * (1 - (1 + self.alpha / 2) * bkd.from_numpy(np.sum( bkd.to_numpy(x ** 2),axis=1)))
            )
            return lhs - rhs
        return fpde
    ...
    def gen_data(self): 
        bc = dde.icbc.DirichletBC(self.geomtime, self.sol, lambda _, on_boundary: on_boundary)
        return dde.data.FPDE(self.geomtime, self.pde, self.alpha, bc, [8,100], num_domain=self.NumDomain, num_boundary=1, solution=self.sol)
